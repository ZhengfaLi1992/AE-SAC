...
if __name__ == "__main__":
    ...
    # defender_agent.actor_network.model.save_weights("models/defender_agent_awid_model.h5", overwrite=True)
    # with open("models/defender_agent_awid_model.json", "w") as outfile:
    #     json.dump(defender_agent.actor_network.model.to_json(), outfile)

    with open("models/opt-awid/defender_agent_awid_model.json", "r") as jfile:
        model = model_from_json(json.load(jfile))
    model.load_weights("models/opt-awid/defender_agent_awid_model.h5")
    ...
    cmm = ConfusionMatrix(4, ['normal', 'flooding', 'injection', 'impersonation'])
    cmm.update(actions, aggregated_data_test)
    cmm.summary()
    cmm.plot('AWID')